...
import {
  setupTxExtension,
  TxClient,
  createFanfuryProtobufRpcClient,
} from './tx';
import {
  FanfuryClientOptions,
...
/**
 * The main entry point for interacting with the FanFury Blockchain. The class needs
 * a client connection
 */
export class FanfuryClient<T extends object> {
  ...
  private _clientOptions!: FanfuryClientOptions;
      ...
      switchMap(() => {
        ...
        for (const moduleName in this._modules) {
          this._modules[moduleName].prototype.setHeight = (height: number) => {
            const queryClient = createFanfuryProtobufRpcClient(
  ...
  /**
   * Create a FanfuryClient object which connects to the given gRPC connection.
   *
   * @param options - Options to pass into FanfuryClient.
   */
    ...
    switch (connection.type) {
      case 'tendermint':
        ...
        */
        ...
        const rpcClient = createFanfuryProtobufRpcClient(queryClient);
