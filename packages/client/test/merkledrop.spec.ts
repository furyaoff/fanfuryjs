...
import { AccountWithProofs, Merkledrop } from '@fanfuryjs/utils';
import { FanfuryClient } from '../dist/client';
import { MsgCreate, MsgClaim } from '../dist/codec/fanfury/merkledrop/v1beta1/tx';
import {
  TEST_ADDRESS,
  TEST_MNEMONIC,
  OTHER_TEST_ADDRESS,
...
let api: FanfuryClient<typeof modules>;
let apiOther: FanfuryClient<typeof modules>;
let merkledrop: Merkledrop;

let merkledropId: string;

describe('FanFuryApi Merkledrop with tendermint connection', () => {
  beforeAll(async () => {
      ...
      if (signedTxBytes) {
        ...
        if (txRes) {
          ...
          const merkledropIdAttr = logs.findAttribute(
            parsedLogs,
            'fanfury.merkledrop.v1beta1.EventCreate',
            'merkledrop_id',
          );
