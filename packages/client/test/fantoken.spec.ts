...
import {
  ...
  MsgSetAuthority,
  MsgSetUri,
} from '../dist/codec/fanfury/fantoken/v1beta1/tx';
import {
  TEST_ADDRESS,
  OTHER_TEST_ADDRESS,
  TEST_MNEMONIC,
...
describe('FanFuryApi Fantoken with tendermint connection', () => {
  beforeAll(async () => {
      ...
      if (signedTxBytes) {
        ...
        if (txRes) {
          ...
          const denomAttr = logs.findAttribute(
            parsedLogs,
            'fanfury.fantoken.v1beta1.EventIssue',
            'denom',
          );
